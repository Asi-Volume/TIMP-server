...
void Database::createTables()
{
    ...
    if (checkQuery.value(0).toInt() == 0) {
        query.prepare("INSERT INTO tasks (task_number, variant, correct_answer) VALUES (1, 1, '5')");
        query.exec();
        query.prepare("INSERT INTO tasks (task_number, variant, correct_answer) VALUES (1, 2, '7')");
        query.exec();
        query.prepare("INSERT INTO tasks (task_number, variant, correct_answer) VALUES (2, 1, '12')");
        query.exec();
        query.prepare("INSERT INTO tasks (task_number, variant, correct_answer) VALUES (2, 2, '20')");
        query.exec();
    }
}
...
{
    QSqlQuery query;
    query.prepare("SELECT correct_answer FROM tasks WHERE task_number = :task_number AND variant = :variant");
...
bool Database::checkSolution(const QString& login, int taskNumber, int variant, const QString& answer)
{
...
void Database::updateStats(const QString& login, bool isCorrect)
{
    QSqlQuery query;
    if (isCorrect) {
        query.prepare("UPDATE users SET total_solved = total_solved + 1, total_attempts = total_attempts + 1, rating = rating + 10 WHERE login = :login");
    }
    else {
        query.prepare("UPDATE users SET total_attempts = total_attempts + 1, rating = rating - 5 WHERE login = :login");
    }
    query.bindValue(":login", login);
    query.exec();
}